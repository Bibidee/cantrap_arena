...
Arena: `create_challenge`, `bind_vault` (deployer-authorized one-time binding when not atomically initialized), `notify_funded`/`sync_funding` (Vault-only and idempotent), `activate`, `commit_attack`, `reveal_attack`, `test_attack`, `expire`, `get_challenge`, `get_attack`, `get_challenge_count`, `get_attack_count`, `get_active_attack_id`.
...
Each challenge maintains its own pending attack ID list. Commit adds an attack, testing removes it, and expiry checks only that challenge's pending windows; expiry never scans unrelated global attacks.
